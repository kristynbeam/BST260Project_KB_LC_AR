...
aus_total_high <- aus_total_high %>% 
    group_by(State) %>% 
    arrange(Date) %>% 
    mutate(Cases_7dayavg = rollmean(Cases, 
                                    k = 7, 
                                    fill = NA)) %>% 
    mutate(Rates_7dayavg = rollmean(Rates, 
                                    k = 7, 
                                    fill = NA)) %>% 
    ungroup()
...
# Figure out maximum and minimum values of count and rate for plots in Shiny app
...
# Do this with our data, Cases and Rates data

# Created a merged dataset between x and aus_total_high
merge_dat <- x %>% 
  rename(State = NAME) %>% 
...
```{r}
aus_total_high_reg <- aus_total_high %>% 
  filter(Disease == "Influenza (laboratory confirmed)") %>%
...
linearMod <- lm(aus_total_high_reg$year1 ~ aus_total_high_reg$year2) 
confint.lm(linearMod)
summary(linearMod)
```
